...
public class Repository {
    ...
    private static void checkoutWithBranchName(String branchName) {
        // Convert to remote branch name if necessary
        branchName = convertRemoteBranchName(branchName);

        // Perform untracked files check before any operation
        checkUntrackedFileError();
        ...
        // Fetch the commit at the head of the given branch
        String commitID = readObject(join(BRANCH_FOLDER, branchName), Pointer.class).getCommitID();
        Commit targetCommit = readObject(join(COMMITS_FOLDER, commitID), Commit.class);

        // Fetch the current commit for comparison
        Commit currentCommit = getCurrentCommit();

        // Delete files not in the target commit but present in the current commit
        for (Blob blob : currentCommit.getBlobs()) {
            String fileName = blob.getCopiedFileName();
            if (!isFilePresentInCommit(targetCommit, fileName)) {
                restrictedDelete(join(CWD, fileName));
            }
        }

        // Overwrite or add files from the target commit
        for (Blob blob : targetCommit.getBlobs()) {
            String fileName = blob.getCopiedFileName();
            checkoutHelper(targetCommit, fileName);
        }

        // Clear the staging area if not the current branch
        clearStaging();
    ...
    private static boolean isFilePresentInCommit(Commit commit, String fileName) {
        for (Blob blob : commit.getBlobs()) {
